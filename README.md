...
|  | GET `/doctors/appointments/upcoming` | Lịch hẹn sắp tới của bác sĩ | — |
...
|  | GET `/medical-records/{id}` | Chi tiết một hồ sơ | Path: `id` |
...
| Khám bệnh & hồ sơ | Doctor | 1) Tạo lịch làm việc → 2) Xem lịch khám sắp tới → 3) Nhận bệnh nhân có appointment → 4) Lập/điều chỉnh hồ sơ → 5) Upload file → 6) Duyệt hồ sơ → 7) Kê đơn | 1) `POST /doctors/schedules` `{ doctorId, dayOfWeek, startTime, endTime, isAvailable? }`.<br>2) `GET /doctors/appointments/upcoming` để lấy lịch khám của bác sĩ.<br>3) appointment liên kết doctorId, có thể `POST /appointments/{id}/confirm|cancel|complete` để cập nhật trạng thái.<br>4) `POST /medical-records` `{ appointmentId, diagnosis, notes }` trả `status=DRAFT` + nếu cần chỉnh sửa thì `PATCH /medical-records/{id}`.<br>5) `POST /medical-records/files/upload` multipart `medicalRecordId`, `file`, `fileType`.<br>6) `POST /medical-records/{id}/approve` → `status=APPROVED` (hồ sơ khóa để đọc).<br>7) `POST /prescriptions` `{ medicalRecordId, medicines:[{ medicineId, dosage, quantity, instructions? }] }`. |
...
- `GET /medical-records/files/medical-record/{medicalRecordId}` và `GET /medical-records/files/{id}/download`: tải danh sách và nội dung file cận lâm sàng.
- `GET /chat/appointments/{appointmentId}/messages` và `/messages/unread`: poll lịch sử chat và tin chưa đọc.
